...
type
    ...
    #     positive_sign : LPSTR
    #     negative_sign : LPSTR
    #     int_frac_digits : LPSTR
    #     frac_digits : LPSTR
    #     p_cs_precedes : LPSTR
    #     p_sep_by_space : LPSTR
    #     n_cs_precedes : LPSTR
    #     n_sep_by_space : LPSTR
    #     p_sign_posn : LPSTR
    #     n_sign_posn : LPSTR
    #     w_decimal_point : LPWSTR
    #     w_thousands_sep : LPWSTR
    #     w_int_curr_symbol : LPWSTR
    #     w_currency_symbol : LPWSTR
    #     w_mon_decimal_point : LPWSTR
    #     w_mon_thousands_sep : LPWSTR
    #     w_positive_sign : LPWSTR
    #     w_negative_sign : LPWSTR
    # LPLCONV = ptr LCONV
...
proc WideCharToMultiByte(CodePage: UINT, dwFlags: DWORD, lpWideCharStr: LPWCHAR, cchWideChar: INT,
...
proc BeginPaint(hWnd: HWND, lpPaint: LPPAINTSTRUCT): HDC {.stdcall, dynlib: "user32", importc, discardable.}
proc EndPaint(hWnd: HWND, lpPaint: LPPAINTSTRUCT): BOOL {.stdcall, dynlib: "user32", importc, discardable.}
proc DrawFrameControl(P1: HDC, P2: LPRECT, P3: UINT, P4: UINT): BOOL {.stdcall, dynlib: "user32", importc, discardable.}
# End of User32
...
# proc setlocale(category: cint; locale: LPCWSTR): LPWSTR {.importc: "_wsetlocale", header: "locale.h", discardable.}
# proc localeconv(): LPLCONV {.importc, header: "locale.h", discardable.}
...
# Misc functions
let pipeChar = ord('|')
proc toWcharPtrW32(srcString: string): LPCWSTR =
    var wLen: INT = MultiByteToWideChar(CP_UTF8, 0, srcString[0].unsafeAddr, INT(srcString.len), nil, 0) + 1
...
proc wstringToString(wBuffer: wstring): string =
    ...
    if WideCharToMultiByte(CP_UTF8, 0, wBuffer[0].unsafeAddr, iLen, s[0].unsafeAddr, iLen, nil, nil) != iLen:
        return ""
    result = cast[string](s)


proc wcharPtrToString(wArrPtr: LPCWSTR): string =
...
proc toUtf8String(pchar: LPSTR): string =
    let iptr = cast[ByteAddress](pchar) #cast[int](source)
    var index = 0
    var sseq : seq[char] = @[]
    while index < AU3_STR_LIMIT:
        let nxtiptr = iptr + (index * char.sizeof)
        let nxtwcp = cast[ref char](nxtiptr)
        if ord(nxtwcp[]) == 0: break
        sseq.add(nxtwcp[])
        inc index
    result = sseq.join()
...
proc `==`(a: WCHAR, b: WCHAR) : bool = ord(a) == ord(b)