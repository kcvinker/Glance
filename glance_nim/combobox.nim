# combobox module Created on 30-Mar-2023 03:23 AM; Author kcvinker [Added to Glance - 01-Jun-2023 23:14]
# ComboBox type
...
proc newComboBox*(parent: Form, x, y, w, h: int32): ComboBox {.exportc:"newComboBox", stdcall, dynlib.} =
    new(result)
...
proc removeItem*(this: ComboBox, index: int32) {.exportc:"removeComboItem2", stdcall, dynlib.} =
    if this.mIsCreated and index > -1:
        this.sendMsg(CB_DELETESTRING, index, 0)
        this.mItems.delete(index)

proc removeAll*(this: ComboBox) {.exportc:"removeAllComboItems", stdcall, dynlib.}=
    if this.mItems.len > 0:
...
proc selctedItem*(this: ComboBox): LPCWSTR =
    if this.mIsCreated:
        this.mSelIndex = int32(this.sendMsg(CB_GETCURSEL, 0, 0))
...
proc setComboBoxEvents(this: ComboBox, evtIndex: ComboBoxEvents, funcPtr: EventHandler) =
    case evtIndex
    of cmbSelectionChanged: this.onSelectionChanged = funcPtr
    of cmbTextChanged: this.onTextChanged = funcPtr
    of cmbTextUpdated: this.onTextUpdated = funcPtr
    of cmbListOpened: this.onListOpened = funcPtr
    of cmbListClosed: this.onListClosed = funcPtr
    of cmbSelectionCommitted: this.onSelectionCommitted = funcPtr
    of cmbSelectionCancelled: this.onSelectionCancelled = funcPtr

proc setComboBoxProps(this: ComboBox, prop: ComboBoxProps, value: pointer) =
    case prop
    of cmbHasInput: this.hasInput = cast[bool](cast[ref int](value)[])
    of cmbSelectedIndex: this.selectedIndex = cast[ref int32](value)[]
    of cmbSelctedItem: this.selctedItem = cast[LPWCHAR](value)

proc getComboBoxProps(this: ComboBox, prop: ComboBoxProps, value: pointer) =
    case prop
    of cmbHasInput: cast[ref bool](value)[] = this.hasInput
    of cmbSelectedIndex: cast[ref int32](value)[] = this.selectedIndex
    of cmbSelctedItem: cast[ref LPWCHAR](value)[] = this.selctedItem
