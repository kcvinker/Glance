# progressbar module Created on 09-Apr-2023 01:42 AM; Author kcvinker[Added to Glance on 03-Jun-2023 20:23]
# ProgressBar type
...
# ProgressBar constructor w, h = 200, 25
proc newProgressBar*(parent: Form, x, y, w, h: int32): ProgressBar {.exportc:"newProgressBar", stdcall, dynlib.} =
    new(result)
...
method createHandle*(this: ProgressBar) =
    this.setPbStyle()
...
proc startMarquee*(this: ProgressBar) {.exportc:"progressBarStartMarquee", stdcall, dynlib.} =
    if this.mIsCreated and this.mBarStyle == pbsMarquee:
        this.sendMsg(PBM_SETMARQUEE, 1, this.mMarqueeSpeed)

# Stop the marquee animation
proc stopMarquee*(this: ProgressBar) {.exportc:"progressBarStopMarquee", stdcall, dynlib.} =
    if this.mIsCreated and this.mBarStyle == pbsMarquee:
...
# proc setProgressBarEvents(this: ProgressBar, evtIndex: ProgressBarEvents, funcPtr: EventHandler) =
#     this.onProgressChanged = funcPtr

proc setProgressBarProps(this: ProgressBar, prop: ProgressBarProps, value: pointer) =
    case prop
    of pgbValue: this.value = cast[ref int32](value)[]
    of pgbStep: this.step = cast[ref int32](value)[]
    of pgbMarqueeSpeed: this.marqueeSpeed = cast[ref int32](value)[]
    of pgbStyle: this.style = cast[ProgressBarStyle](cast[ref int](value)[])
    of pgbState: this.state = cast[ProgressBarState](cast[ref int](value)[])
    of pgbShowPercentage: this.showPercentage = cast[bool](cast[ref int](value)[])

proc getProgressBarProps(this: ProgressBar, prop: ProgressBarProps, value: pointer) =
    case prop
    of pgbValue: cast[ref int32](value)[] = this.value
    of pgbStep: cast[ref int32](value)[] = this.step
    of pgbMarqueeSpeed: cast[ref int32](value)[] = this.marqueeSpeed
    of pgbStyle: cast[ref int32](value)[] = int32(this.style)
    of pgbState: cast[ref int32](value)[] = int32(this.state)
    of pgbShowPercentage: cast[ref bool](value)[] = this.showPercentage
    ...
    of WM_PAINT:
        if this.mShowPerc and this.mBarStyle == pbsBlock:
            ...
            if this.onPaint != nil:
                var ps : PAINTSTRUCT
                ps.hdc = hdc
                GetClientRect(hw, ps.rcPaint.unsafeAddr)
