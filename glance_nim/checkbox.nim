# checkbox module Created on 29-Mar-2023 11:23 PM; Author kcvinker[ Added to Glance on 01-Jun-2023 05:07]
# CheckBox type
...
proc newCheckBox*(parent: Form, text: LPCWSTR, x, y, w, h: int32): CheckBox {.exportc:"newCheckBox", stdcall, dynlib.} =
    new(result)
...
method createHandle*(this: CheckBox) =
    this.setCbStyle()
    this.createHandleInternal()
    if this.mHandle != nil:
        this.setSubclass(cbWndProc)
        this.setFontInternal()
        this.setSizeWithText()
...
proc setCheckBoxEvents(this: CheckBox, evtIndex: CheckBoxEvents, funcPtr: EventHandler) =
    this.onCheckedChanged = funcPtr

proc setCheckBoxProps(this: CheckBox, prop: CheckBoxProps, value: pointer) =
    this.checked = cast[bool](cast[ref int](value)[])

proc getCheckBoxProps(this: CheckBox, prop: CheckBoxProps, value: pointer) =
    cast[ref bool](value)[] = this.checked
...
proc cbWndProc(hw: HWND, msg: UINT, wpm: WPARAM, lpm: LPARAM, scID: UINT_PTR, refData: DWORD_PTR): LRESULT {.stdcall.} =
    ...
    of MM_NOTIFY_REFLECT:
        ...
        of CDDS_PREPAINT:
            ...
            DrawTextW(nmcd.hdc, this.mText[0].unsafeAddr, -1, nmcd.rc.unsafeAddr, this.mTextStyle)
