...
# Button constructor
proc newButton*(parent: Form, txt: LPCWSTR, x: int32 = 10, y: int32 = 10, w: int32 = 110, h: int32 = 34): Button {.exportc:"gflNewButton", stdcall, dynlib.} =
    new(result)
...
proc setBackColor(this: Button, clr: uint) =
    this.mBackColor = newColor(clr)
    this.mFDraw.flatDrawSetData(this.mBackColor)
    if (this.mDrawMode and 2) != 2: this.mDrawMode += 2
    this.checkRedraw()
...
proc setBtnGradientColor*(this: Button, clr1, clr2: uint) {.exportc:"BtnSetGradientColor", stdcall, dynlib.} =
...
proc drawTextColor(this: Button, ncd: LPNMCUSTOMDRAW): LRESULT =
    ...
    SetBkMode(ncd.hdc, 1)
    DrawTextW(ncd.hdc, this.mText[0].unsafeAddr, -1, ncd.rc.unsafeAddr, this.mTxtFlag )
...
proc drawBackColor(this: Button, ncd: LPNMCUSTOMDRAW): LRESULT =
    ...
    of CDDS_PREERASE: return CDRF_NOTIFYPOSTERASE # Telling the program to inform us after erase
...
proc createBtnHwnds(btns: varargs[Button]) {.exportc:"createBtnHwnds123", stdcall, dynlib.} =

    var ba = cast[ByteAddress](btns[0])
    var sba = cast[Button](ba + 624)
    var sb = sba[]
    echo "starting man - ", sb.mName
