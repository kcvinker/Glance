...
proc setFormPosition(this: Form) =
    if this.mXpos == 0 and this.mYpos == 0:
        case this.mFormPos
        of fpCenter:
...
proc frmPaintHandler(this: Form, hw: HWND, msg: UINT, wpm: WPARAM, lpm: LPARAM) : LRESULT =
    if this.onPaint != nil:
        var ps : LPPAINTSTRUCT
        BeginPaint(hw, ps)
        this.onPaint(this, newPaintEventArgs(ps))
        EndPaint(hw, ps)
        return 1
...
proc frmOnMouseUp(c: Control, mea: MouseEventArgs) {.stdcall.} = echo "X: ", $mea.mx, "  Y: ", $mea.my


proc newForm(title: LPCWSTR, width: int32, height: int32,
             x: int32 = 0, y: int32 = 0 ): Form {.exportc:"getNewForm", stdcall, dynlib.}  =
    new(result)
    ...
    result.mXpos = x
    result.mYpos = y
    result.mFont = newFont(toWstring("Tahoma"), 11)
    ...
    result.mText = toWstring(title) #(if title == "": "Form_" & $appData.formCount else: title)
...
proc close*(this: Form) {.exportc:"frmClose", stdcall, dynlib.} = DestroyWindow(this.mHandle)

proc setGradientBackColor*(this: Form, clr1, clr2 : uint, rtl: bool) {.exportc:"frmSetGradient", stdcall, dynlib.} =
...
# Functions for glance dll
proc setFormEvents(this: Form, evt: FormEvents, funcPtr: EventHandler) =
    case evt
    of feLoad: this.onLoad = funcPtr
    of feMaximize: this.onMaximized = funcPtr
    of feMinimize: this.onMinimized = funcPtr
    of feRestore: this.onRestored = funcPtr
    of feClosing: this.onClosing = funcPtr
    of feActivate: this.onActivate = funcPtr
    of feDeActivate: this.onDeActivate = funcPtr
    of feMoving: this.onMoving = funcPtr
    of feMoved: this.onMoved = funcPtr
    of feSizing: this.onSizing = cast[SizeEventHandler](funcPtr)
    of feSized: this.onSized = cast[SizeEventHandler](funcPtr)


proc setFormProps(this: Form, prop: FormProps, value: pointer) =
    case prop
    of fpFormPos: this.startPos = cast[FormPos](cast[ref int](value)[])
    of fpFormState: this.formState = cast[WindowState](cast[ref int](value)[])
    of fpFormStyle: this.formStyle = cast[FormStyle](cast[ref int](value)[])
    of fpTopMost: this.topMost = cast[bool](cast[ref int](value)[])
    of fpMaxBox: this.maximizeBox = cast[bool](cast[ref int](value)[])
    of fpMinBox: this.minimizeBox = cast[bool](cast[ref int](value)[])

proc getFormProps(this: Form, prop: FormProps, value: pointer) =
    case prop
    of fpFormPos: cast[ref int32](value)[] = int32(this.mFormPos)
    of fpFormState: cast[ref int32](value)[] = int32(this.formState)
    of fpFormStyle: cast[ref int32](value)[] = int32(this.formStyle)
    of fpTopMost: cast[ref bool](value)[] = this.topMost
    of fpMaxBox: cast[ref bool](value)[] = this.maximizeBox
    of fpMinBox: cast[ref bool](value)[] = this.minimizeBox

proc setMenuEvents(this: Form, menuName: LPCWSTR, eventIndex: int, funcPtr: MenuEventHandler) {.
                        exportc:"setMainMenuEvents", stdcall, dynlib.} =
...
proc mainWndProc( hw: HWND, msg: UINT, wpm: WPARAM, lpm: LPARAM): LRESULT {.stdcall.} =
    ...
    of WM_PAINT: return this.frmPaintHandler(hw, msg, wpm, lpm)
    ...
    of WM_KEYUP, WM_SYSKEYUP:
        if this.onKeyUp != nil: this.onKeyUp(this, newKeyEventArgs(wpm, true, this))

    of WM_KEYDOWN, WM_SYSKEYDOWN:
        if this.onKeyDown != nil: this.onKeyDown(this, newKeyEventArgs(wpm, false, this))
    ...
    of WM_CTLCOLORLISTBOX:
        let hList = cast[HWND](lpm)
        let cmbHwnd = this.mComboData.getOrDefault(hList)
        if cmbHwnd != nil:
            return SendMessageW(cmbHwnd, MM_LIST_COLOR, wpm, lpm)
        else:
            return SendMessageW(hList, MM_LIST_COLOR, wpm, lpm)
    ...
    of WM_MEASUREITEM:
        ...
        var hdc = GetDC(hw)
        var size : SIZE
        GetTextExtentPoint32(hdc, mi.mText[0].unsafeAddr, int32(len(mi.mText)), size.unsafeAddr)
        ReleaseDC(hw, hdc)
        if mi.mType == mtBaseMenu:
            pmi.itemWidth = UINT(size.cx) #+ 10
            pmi.itemHeight = UINT(size.cy)
        else:
            pmi.itemWidth = UINT(size.cx + 15) #+ 10
            pmi.itemHeight = UINT(size.cy + 10)
        return 1
    ...
    of WM_UNINITMENUPOPUP:
        var menu = this.getMenuFromHmenu(cast[HMENU](wpm))
        if menu != nil and menu.onCollapse != nil : menu.onCollapse(menu, newEventArgs())

    # of WM_ENTERMENULOOP: echo "WM_ENTER MENULOOP"
    # of WM_EXITMENULOOP: echo "WM_EXIT MENULOOP"

    of WM_COMMAND:
        case HIWORD(wpm)
        of 0:
            if len(this.mMenuItemDict) > 0 and int(lpm) == 0:
                var menu = this.mMenuItemDict.getOrDefault(int32(LOWORD(wpm)))
                if menu != nil and menu.onClick != nil:
                    menu.onClick(menu, newEventArgs())
                return 0
        else: discard
