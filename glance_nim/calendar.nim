...
proc newCalendar*(parent: Form, x: int32 = 10, y: int32 = 10): Calendar {.exportc:"newCalendar", stdcall, dynlib.} =
    new(result)
...
method createHandle*(this: Calendar) =
    this.setCalStyle()
...
# method createHwnd(this: Calendar) = this.createHandle()
...
# proc calValueSetter(this: Calendar, dtVal: str)

proc setCalEvents(this: Calendar, evtIndex: CalendarEvents, funcPtr: EventHandler) =
    case evtIndex:
    of calSelectionCommitted: this.onSelectionCommitted = funcPtr
    of  calValueChanged: this.onValueChanged = funcPtr
    of calViewChanged: this.onViewChanged = funcPtr

proc setCalProps(this: Calendar, prop: CalendarProps, value: pointer) =
    case prop
    of calValue: this.value = (cast[ref DateAndTime](value))[]
    of calViewMode: this.viewMode = cast[ViewMode](cast[ref int](value)[])
    of calShowWeekNumber: this.showWeekNumber = cast[bool](cast[ref int](value)[])
    of calNoTodayCircle: this.noTodayCircle = cast[bool](cast[ref int](value)[])
    of calNoToday: this.noToday = cast[bool](cast[ref int](value)[])
    of calNoTrailingDates: this.noTrailingDates = cast[bool](cast[ref int](value)[])
    of calShortDateNames: this.shortDateNames = cast[bool](cast[ref int](value)[])

proc getCalProps(this: Calendar, prop: CalendarProps, value: pointer) =
    case prop
    of calValue: cast[ref DateAndTime](value)[] = this.value
    of calViewMode: cast[ref int32](value)[] = int32(this.viewMode)
    of calShowWeekNumber: cast[ref bool](value)[] = this.showWeekNumber
    of calNoTodayCircle: cast[ref bool](value)[] = this.noTodayCircle
    of calNoToday: cast[ref bool](value)[] = this.noToday
    of calNoTrailingDates: cast[ref bool](value)[] = this.noTrailingDates
    of calShortDateNames: cast[ref bool](value)[] = this.shortDateNames

proc glfNewDateTime(iYear, iMonth, iDay, iHour, iMinute, iSecond, iMillisecond, iDoW: int32 ): ref DateAndTime {.exportc:"glfNewDateTime", stdcall, dynlib.} =
    new(result)
    result.year = iYear
    result.month = iMonth
    result.day = iDay
    result.hour = iHour
    result.minute = iMinute
    result.second = iSecond
    result.milliSecond = iMillisecond
    result.dayOfWeek = cast[WeekDays](iDoW)
...
# var dY, dM, dD, dH, dMn, dS,
# let ms = chr(32)
# let ds = chr(45)
# let ts = chr(58)
# let inStr = "12-05-2023"
# let p1 = r"\d\d?" & ds & r"\d\d?" & ds & r"\d{2, 4}"
# let p2 = r"\d\d?" & ts & "\d\d?" & ts & "\d\d?"
# var mains = split(inStr, ms)
# case mains.len
# of 1:
#   var tm = split(mains[0], ds)
