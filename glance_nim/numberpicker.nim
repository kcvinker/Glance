# numberpicker module Created on 03-Apr-2023 01:57 PM; Author kcvinker [Added to Glance on 03-Jun-2023 21:56]
# NumberPicker type
...
proc newNumberPicker*(parent: Form, x, y, w, h: int32): NumberPicker {.exportc:"newNumberPicker", stdcall, dynlib.} =
    new(result)
...
proc setNPStyle(this: NumberPicker) =
    ...
    this.mPen = CreatePen(PS_SOLID, 1, this.mBackColor.cref)
...
proc displayValue(this: NumberPicker) =
    if this.mDeciPrec > 0:
        this.mBuddyStr = toWstring(this.mValue.formatFloat(ffDecimal, this.mDeciPrec))
    else:
        this.mBuddyStr = toWstring($(int(this.mValue)))
    SetWindowTextW(this.mBuddyHandle, this.mBuddyStr[0].unsafeAddr)
...
proc setValueInternal(this: NumberPicker, delta: int32) =
    let newValue : float = this.mValue + (float(delta) * this.mStep)
    if this.mAutoRotate:
...
method createHandle*(this: NumberPicker) =
    this.setNPStyle()
...
proc `value=`*(this: NumberPicker, fValue: auto) = # Accepts int or float
    this.mValue = (if this.mDeciPrec > 0: float(fValue) else: fValue)
    if this.mIsCreated: this.displayValue()

proc value*(this: NumberPicker): float = this.mValue
...
proc `step=`*(this: NumberPicker, value: float) = this.mStep = value

template step*(this: NumberPicker): float = this.mStep

proc `decimalDigits=`*(this: NumberPicker, value: int32) = this.mDeciPrec = value
proc decimalDigits*(this: NumberPicker): int32 = this.mDeciPrec
...
proc setNumberPickerProps(this: NumberPicker, prop: NumberPickerProps, value: pointer) =
    case prop
    of npValue:this.value = cast[ref float](value)[]
    of npStep: this.step = cast[ref float](value)[]
    of npMinRange: this.minRange = cast[ref float](value)[]
    of npMaxRange: this.maxRange = cast[ref float](value)[]
    of npButtonLeft: this.buttonLeft = cast[bool](cast[ref int](value)[])
    of npAutoRotate: this.autoRotate = cast[bool](cast[ref int](value)[])
    of npHideCaret: this.hideCaret = cast[bool](cast[ref int](value)[])
    of npTextAlign: this.textAlign = cast[TextAlignment](cast[ref int](value)[])
    of npDecimalDigits: this.decimalDigits = cast[ref int32](value)[]

proc getNumberPickerProps(this: NumberPicker, prop: NumberPickerProps, value: pointer) =
    case prop
    of npValue: cast[ref float](value)[] = this.value
    of npStep: cast[ref float](value)[] = this.step
    of npMinRange: cast[ref float](value)[] = this.minRange
    of npMaxRange: cast[ref float](value)[] = this.maxRange
    of npButtonLeft: cast[ref bool](value)[] = this.buttonLeft
    of npAutoRotate: cast[ref bool](value)[] = this.autoRotate
    of npHideCaret: cast[ref bool](value)[] = this.hideCaret
    of npTextAlign: cast[ref int32](value)[] = int32(this.textAlign)
    of npDecimalDigits: cast[ref int32](value)[] = this.decimalDigits
